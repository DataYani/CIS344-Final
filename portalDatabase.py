...
class Database():
    ...
    def scheduleAppointment(self, patient_id, doctor_id, appointment_date, appointment_time):
        ...
        if self.connection.is_connected():
            self.cursor = self.connection.cursor()
            query = "CALL CreateAppointment(%s, %s, %s, %s)"
    ...
    def viewAppointments(self):
        ...
        if self.connection.is_connected():
            self.cursor = self.connection.cursor()
            query = "SELECT * FROM DoctorsAppointmentsPatientsView"
            self.cursor.execute(query)
            records = self.cursor.fetchall()
            return records
    ...
    def dischargePatient(self, patient_id):
        ...
        if self.connection.is_connected():
            self.cursor = self.connection.cursor()
            query = "CALL DischargePatient(%s, CURDATE())"
            self.cursor.execute(query, (patient_id,))
            self.connection.commit()
            return

    def updatePatientDetails(self, patient_id, new_name, new_age, new_admission_date, new_discharge_date):
        ''' Method to update patient details '''
        if self.connection.is_connected():
            self.cursor = self.connection.cursor()
            query = "UPDATE patients SET patient_name=%s, age=%s, admission_date=%s, discharge_date=%s WHERE patient_id=%s"
            self.cursor.execute(query, (new_name, new_age, new_admission_date, new_discharge_date, patient_id))
            self.connection.commit()
            return
    ...
    def viewRecords(self):
        ''' Method to view all records (joining tables) '''
        if self.connection.is_connected():
            self.cursor = self.connection.cursor()
            query = "SELECT * FROM DoctorsAppointmentsPatientsView"
            self.cursor.execute(query)
